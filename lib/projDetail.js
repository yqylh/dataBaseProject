exports.projDetail = async(req, res) =>{
            ...
            return new Promise((resolve, reject)=>{
                if (proj.article_id != null) {
                    mysql.article_fileQuery(proj.article_id, (ans)=>{
                        ans.forEach(element => {
                            let name, path = element.address;
                            for (k = path.length - 1; k >= 0; k--) if (path[k] == '/') {
                                name = path.substr(k + 1, path.length - k);
                                break;
                            }
                            obj.file.push({id:element.file_id, name:name});
                        });
                        resolve(proj);
                    })
                } else {
